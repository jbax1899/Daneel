...
export class AudioPlaybackHandler {
    ...
    pipelineCleanupTimers = new Map();
    async playAudioToChannel(connection, audioData) {
    ...
    ensurePipeline(connection) {
        ...
        pipeline.getOpusEncoder().once('error', (error) => {
    ...
    async processAudioQueue(connection) {
        const guildId = connection.joinConfig.guildId;
        const queue = this.audioQueues.get(guildId);
        if (!queue || queue.length === 0) {
    ...
    clearPipelineCleanupTimer(guildId) {
    ...
    schedulePipelineCleanup(guildId) {
    ...
    retryProcessingQueue(connection) {
        const guildId = connection.joinConfig.guildId;
        const timer = setTimeout(() => {
            const queue = this.audioQueues.get(guildId);
            if (!queue || queue.length === 0) {
    ...
    cleanupPipeline(guildId) {
        const pipeline = this.pipelines.get(guildId);
        if (pipeline) {
            void pipeline.destroy().catch((error) => {
...
//# sourceMappingURL=AudioPlaybackHandler.js.map