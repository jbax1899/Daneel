...
import { cleanupVoiceConnection } from '../events/VoiceStateHandler.js';
/*
* @name call
...
const callCommand = {
    data: new SlashCommandBuilder()
        ...
        .addChoices({ name: 'Exit', value: 'exit' }, // Terminate the voice connection for this server/guid
    { name: 'ExitAll', value: 'exit_all' } // Terminate all voice connections
    )
        .setRequired(false)),
    async execute(interaction) {
        // Store the voice channel we want to use
        const voiceChannel = interaction.options.getChannel('channel', true, [ChannelType.GuildVoice]);
        ...
        if (debugOption) {
            switch (debugOption) {
                case 'exit':
                    const existingVoiceConnection = getVoiceConnection(interaction.guild.id);
                    ...
                    return;
                case 'exit_all':
                    const existingVoiceConnections = getVoiceConnections();
