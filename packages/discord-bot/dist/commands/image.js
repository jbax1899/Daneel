...
const imageCommand = {
    ...
    async execute(interaction) {
        ...
        let aspect_ratio = interaction.options.getString('aspect_ratio');
        if (aspect_ratio) {
            switch (aspect_ratio) {
                case 'square':
        ...
        // Get parameters
        const quality = interaction.options.getString('quality') || 'low';
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Prompt', value: prompt }, { name: 'Adjusted Prompt', value: `...` }, { name: 'Size', value: dimensions !== 'auto' ? `${interaction.options.getString('aspect_ratio')} (${dimensions}px)` : 'auto', inline: true }, { name: 'Quality', value: quality, inline: true }, { name: 'Background', value: background, inline: true }, { name: 'Model', value: model, inline: true }, { name: 'Ref. Response ID', value: followUpResponseId ? `\`${followUpResponseId}\`` : 'None', inline: true }, { name: 'Output Response ID', value: '...', inline: true })
            .setColor(0x00FF00)
            .setTimestamp()
