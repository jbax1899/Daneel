...
import { IMAGE_VARIATION_CANCEL_CUSTOM_ID_PREFIX, IMAGE_VARIATION_ASPECT_SELECT_PREFIX, IMAGE_VARIATION_GENERATE_CUSTOM_ID_PREFIX, IMAGE_VARIATION_PROMPT_INPUT_ID, IMAGE_VARIATION_PROMPT_MODAL_ID_PREFIX, IMAGE_VARIATION_QUALITY_SELECT_PREFIX, IMAGE_VARIATION_RESET_PROMPT_CUSTOM_ID_PREFIX, IMAGE_VARIATION_PROMPT_ADJUST_SELECT_PREFIX, IMAGE_VARIATION_IMAGE_MODEL_SELECT_PREFIX, EMBED_FIELD_VALUE_LIMIT } from './constants.js';
...
function buildQualityOptions(imageModel) {
...
function buildImageModelOptions(quality) {
    return Object.keys(IMAGE_MODEL_LABELS).map(model => ({
        value: model,
...
const PROMPT_ADJUSTMENT_OPTIONS = [
    {
        value: 'allow',
...
function makeSessionKey(userId, responseId) {
...
export function initialiseVariationSession(userId, responseId, context) {
...
export function getVariationSession(userId, responseId) {
    return sessions.get(makeSessionKey(userId, responseId)) ?? null;
}
export function updateVariationSession(userId, responseId, updater) {
...
export function setVariationSessionUpdater(userId, responseId, updater) {
    const session = getVariationSession(userId, responseId);
    if (!session) {
        return null;
    }
...
export function disposeVariationSession(key) {
...
export function applyVariationCooldown(userId, responseId, seconds) {
    const session = getVariationSession(userId, responseId);
    if (!session) {
        return null;
    }
...
export function resetVariationCooldown(userId, responseId) {
    const session = getVariationSession(userId, responseId);
    if (!session) {
        return null;
    }
...
function buildSelectRow(customId, options, { selectedValue, placeholder, currentLabel }) {
...
export function buildVariationConfiguratorView(session, options = {}) {
    ...
    const rows = [
        buildSelectRow(`${IMAGE_VARIATION_PROMPT_ADJUST_SELECT_PREFIX}${session.responseId}`, PROMPT_ADJUSTMENT_OPTIONS, {
        ...
        buildSelectRow(`${IMAGE_VARIATION_IMAGE_MODEL_SELECT_PREFIX}${session.responseId}`, imageModelOptions, {
            selectedValue: session.imageModel,
            placeholder: 'Select image model',
            currentLabel: IMAGE_MODEL_LABELS[session.imageModel] ?? session.imageModel
        }),
        buildSelectRow(`${IMAGE_VARIATION_QUALITY_SELECT_PREFIX}${session.responseId}`, qualityOptions, {
            selectedValue: session.quality,
            placeholder: 'Select quality',
            currentLabel: QUALITY_LABELS[session.quality] ?? toTitleCase(session.quality)
        }),
        buildSelectRow(`${IMAGE_VARIATION_ASPECT_SELECT_PREFIX}${session.responseId}`, ASPECT_OPTIONS, {
    ...
    const promptButtons = new ActionRowBuilder().addComponents(editPromptButton, resetPromptButton, actionButton, cancelButton);
...
export function buildPromptModal(responseId, currentPrompt) {
    ...
    return modal.addComponents(new ActionRowBuilder().addComponents(promptInput));
}
//# sourceMappingURL=variationSessions.js.map