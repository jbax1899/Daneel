...
import { RealtimeContextBuilder } from '../utils/prompting/RealtimeContextBuilder.js';
export class VoiceStateHandler extends Event {
    sessionManager;
    audioCaptureHandler;
    audioPlaybackHandler;
    userVoiceStateHandler;
    connectionManager;
    client;
    ...
    async execute(oldState, newState) {
        try {
            if (newState.member?.id === this.client.user?.id) {
                ...
                await this.userVoiceStateHandler.handleUserVoiceChange(oldState, newState, this.client, this.startConversation.bind(this));
            }
        }
        catch (error) {
    ...
    async handleBotVoiceStateChange(oldState, newState) {
    ...
    async handleBotJoinedChannel(newState) {
        ...
        const session = this.sessionManager.createSession(connection, realtimeSession, this.audioCaptureHandler, this.audioPlaybackHandler, participantMap, this.userVoiceStateHandler.getInitiatingUser(guildId));
    ...
    collectVoiceParticipants(channel) {
    ...
    async createSession(guildId, channelId) {
        if (this.sessionManager.hasSession(guildId)) {
    ...
    async handleBotLeftChannel(oldState) {
    ...
    async startConversation(guildId) {
        ...
        if (!session) {
            ...
            const realtimeSession = await this.createRealtimeSession(guildId, contextParticipants);
            session = this.sessionManager.createSession(connection, realtimeSession, this.audioCaptureHandler, this.audioPlaybackHandler, participantMap, this.userVoiceStateHandler.getInitiatingUser(guildId));
            this.sessionManager.addSession(guildId, session);
            this.audioCaptureHandler.setupAudioCapture(connection, realtimeSession, guildId);
        }
        try {
    ...
    async createRealtimeSession(guildId, participants) {
        this.audioCaptureHandler.cleanupGuild(guildId);
        ...
        if (!realtimeSession.listenersAttached) {
            realtimeSession.listenersAttached = true;
            realtimeSession.on('audio', (audioData) => {
            ...
            realtimeSession.on('text', (text) => logger.debug(`[BOT TEXT] ${text}`));
            realtimeSession.on('greeting', (text) => logger.info(`[BOT GREETING] ${text}`));
            realtimeSession.on('response.completed', (event) => logger.debug(`[BOT RESPONSE COMPLETED] Response ID: ${event?.response_id || 'unknown'}`));
            realtimeSession.on('response.output_audio.done', (event) => logger.debug('[BOT AUDIO DONE] Audio stream completed for ' + event));
            realtimeSession.on('error', (error) => logger.error('[RealtimeSession] Error:', error));
            realtimeSession.on('connected', () => logger.info('[RealtimeSession] Connected to OpenAI Realtime API'));
        }
        await realtimeSession.connect();
        return realtimeSession;
    }
    removeRealtimeSessionListeners(session) {
        session.removeAllListeners();
    }
    async cleanupExistingConnections() {
        await this.connectionManager.cleanupExistingConnections(this.client);
    }
}
export async function cleanupVoiceConnection(connection, client) {
...
//# sourceMappingURL=VoiceStateHandler.js.map