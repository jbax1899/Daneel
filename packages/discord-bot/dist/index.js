...
import { IMAGE_RETRY_CUSTOM_ID_PREFIX, IMAGE_VARIATION_ASPECT_SELECT_PREFIX, IMAGE_VARIATION_CANCEL_CUSTOM_ID_PREFIX, IMAGE_VARIATION_GENERATE_CUSTOM_ID_PREFIX, IMAGE_VARIATION_PROMPT_INPUT_ID, IMAGE_VARIATION_PROMPT_MODAL_ID_PREFIX, IMAGE_VARIATION_QUALITY_SELECT_PREFIX, IMAGE_VARIATION_RESET_PROMPT_CUSTOM_ID_PREFIX, IMAGE_VARIATION_PROMPT_ADJUST_SELECT_PREFIX, IMAGE_VARIATION_IMAGE_MODEL_SELECT_PREFIX, IMAGE_VARIATION_CUSTOM_ID_PREFIX } from './commands/image/constants.js';
import { buildImageResultPresentation, clampPromptForContext, createRetryButtonRow, executeImageGeneration, formatRetryCountdown } from './commands/image/sessionHelpers.js';
import { recoverContextFromMessage } from './commands/image/contextResolver.js';
import { applyVariationCooldown, buildPromptModal, buildVariationConfiguratorView, disposeVariationSession, getVariationSession, initialiseVariationSession, resetVariationCooldown, setVariationSessionUpdater, updateVariationSession } from './commands/image/variationSessions.js';
 ...
 */
function buildVariationStatusMessage(userId, base) {
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        if (customId.startsWith(IMAGE_VARIATION_ASPECT_SELECT_PREFIX)) {
            ...
            const session = updateVariationSession(interaction.user.id, responseId, current => {
                const { size, aspectRatio, aspectRatioLabel } = resolveAspectRatioSettings(selected);
                current.size = size;
        ...
        if (customId.startsWith(IMAGE_VARIATION_GENERATE_CUSTOM_ID_PREFIX)) {
            ...
            if (!developerBypass) {
                ...
                if (!spendResult.allowed) {
                    const statusMessage = buildVariationStatusMessage(interaction.user.id, describeTokenAvailability(session.quality, spendResult, session.imageModel));
                    ...
                    if (session.messageUpdater) {
                        try {
                            await session.messageUpdater(buildVariationConfiguratorView(updatedSession, { statusMessage }));
                        }
                        catch (error) {
