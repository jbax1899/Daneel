...
function getNumberEnv(key, defaultValue) {
    ...
    if (!Number.isFinite(parsed) || parsed < 0) {
        logger.warn(`Ignoring invalid numeric value for ${key}: "${value}". Expected a non-negative number; using default (${defaultValue}).`);
        return defaultValue;
    }
    return parsed;
}
 ...
 */
function getStringArrayEnv(key, defaultValue) {
    const value = process.env[key];
    if (!value) {
        return [...defaultValue];
    }
    ...
    if (entries.length === 0) {
        logger.warn(`Ignoring ${key} because it did not contain any valid thread identifiers. Falling back to default (${defaultValue.join(', ') || 'none'}).`);
        return [...defaultValue];
    }
    return entries;
}
 ...
 */
function getBotInteractionActionEnv(key, defaultValue) {
    const value = process.env[key];
    if (!value)
        return defaultValue;
    ...
    logger.warn(`Ignoring invalid bot interaction action for ${key}: "${value}". Expected "ignore" or "react"; using default (${defaultValue}).`);
    return defaultValue;
}
/**
...
export const config = {
    ...
    catchUp: {
        ...
        ifMentionedAfterMessages: getNumberEnv('CATCHUP_IF_MENTIONED_AFTER_MESSAGES', DEFAULT_CATCH_UP_LIMITS.IF_MENTIONED_AFTER_MESSAGES),
