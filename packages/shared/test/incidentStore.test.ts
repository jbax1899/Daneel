...
import { SqliteIncidentStore } from '../src/sqliteIncidentStore.js';
import { hmacId } from '../src/pseudonymization.js';
...
test('SqliteIncidentStore pseudonymizes pointers and audit actors', async () => {
  ...
  try {
    ...
    try {
      const stored = db.prepare('SELECT pointers_json FROM incidents WHERE id = ?').get(incident.id);
      const parsed = JSON.parse(stored.pointers_json as string);
    ...
    try {
      const storedAudit = db2.prepare('SELECT actor_hash FROM incident_audit_events WHERE id = ?').get(audit.id);
